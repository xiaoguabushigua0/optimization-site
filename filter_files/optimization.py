...
async def read_item(item: Item): # 参数类型是
    ...
    F = []
    for j in range(4):
        F.append(np.round(lowers_x[0][j] * (4.5 - 0.5) / 251 + 0.5,2))
    for i in range(3):
        F.append(np.round(uppers_x[0][i] * (4.5 - 0.5) / 251 + 0.5,2))
    ...
    x1 = [(L1 - 0.6 - 18) / (48 - 18), (L1 + 0.6 - 18) / (48 - 18)]
    x2 = [(L2 - 0.6 - 15.5) / (48.5 - 15.5), (L2 + 0.6 - 15.5) / (48.5 - 15.5)]
    x3 = [(2 - 2) / (3.2 - 2), (3.2 - 2) / (3.2 - 2)]
    x4 = [(3.2 - 3.2) / (4.7 - 3.2), (4.7 - 3.2) / (4.7 - 3.2)]
    b1 = [1, 1]
    b2 = [1, 1]
    ...
    return {'time':np.round(end_time - start_time),
            ...
            'd23':np.round(variable[0, 3] * (4.7 - 3.2) + 3.2,3), 'F':F}
