...
async def read_item(item: Item): # 参数类型是
    ...
    min_bounds = [(inverse_L1 - 1 - 18) / (48 - 18), (inverse_L2 - 1 - 15.5) / (48.5 - 15.5),
                  0, 0]
    max_bounds = [(inverse_L1 + 1 - 18) / (48 - 18), (inverse_L2 + 1 - 15.5) / (48.5 - 15.5),
                  1, 1]

    pso = PSO(func=mae, dim=4, pop=20, max_iter=15,
              lb=min_bounds, ub=max_bounds, w=0.5, c1=0.5, c2=0.5)
    pso.run()
    print('best_x is ', pso.gbest_x, 'best_y is', pso.gbest_y)
    print(pso.gbest_x[0] * (48 - 18) + 18)
    print(pso.gbest_x[1] * (48.5 - 15.5) + 15.5)
    print(pso.gbest_x[2] * (3.2 - 2) + 2)
    print(pso.gbest_x[3] * (4.7 - 3.2) + 3.2)
    end_time = time.time()
    print('用时：', end_time - start_time, '秒')

    return {'time':np.round(end_time - start_time),
            'L1':np.round(pso.gbest_x[0] * (48 - 18) + 18,3),
            'L2':np.round(pso.gbest_x[1] * (48.5 - 15.5) + 15.5,3),
            'd12':np.round(pso.gbest_x[2] * (3.2 - 2) + 2,3),
            'd23':np.round(pso.gbest_x[3] * (4.7 - 3.2) + 3.2,3), 'F':F_}

if __name__ == '__main__':
    import uvicorn
    uvicorn.run(app, host="192.168.1.101", port=8879)
