...
async def read_item(item: Item): # 参数类型是
    ...
    h.save_report_to_csv("D:/UI_learning/optimizaiton_site/12-25/filter_files/inverse_hfss.csv")

    read1 = np.zeros(251)
    df1 = pd.read_csv("D:/UI_learning/optimizaiton_site/12-25/filter_files/inverse_hfss.csv", header=None)
    DF1 = df1.values
    for i in range(251):
        read1[i] = DF1[i + 1, 1]
    ...
    with plt.style.context(['science', 'no-latex']):
        ...
        ax.legend(['Target', 'Inverse-HFSS'],
                  prop={'size': 14, 'weight': 'roman'}, loc='lower left', facecolor='blue')
        ax.set_xlim(w[0], w[-1])
        ax.set_ylim(min(S11_dB)-7, 0)
        fig.savefig('../opti-site/src/assets/inverse_hfss.svg')

if __name__ == '__main__':
    import uvicorn
    uvicorn.run(app, host="192.168.1.101", port=8883)
